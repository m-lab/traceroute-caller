...
func TestInsertAnnotation(t *testing.T) {
	...
	_, err := parser.InsertAnnotation(fakeAnn, []byte(`xxxx`))
	if err == nil || err.Error() != "Invalid test" {
		t.Error("Fail to detect invalid test")
	}
	...
	if err != nil {
		t.Error("Cannot insert empty annotation")
	}

	fakeAnn["::ffff:180.87.97.101"] = &annotator.ClientAnnotations{
	...
	fakeAnn["::ffff:1.47.236.62"] = &annotator.ClientAnnotations{
		Geo: &annotator.Geolocation{
			ContinentCode: "SA",
		},
		Network: &annotator.Network{
	...
	output, err := parser.InsertAnnotation(fakeAnn, []byte(testStr))
	if err != nil {
		log.Println(err)
	...
	testStr2 := `{"UUID":"ndt-plh7v_1566050090_000000000004D60F","TracerouteCallerVersion":"bc092be","CachedResult":true,"CachedUUID":"ndt-w6lxg_1565921414_00000000000038C0"}
	...
	`
	fakeAnn["2001:550:1b01:1::1"] = &annotator.ClientAnnotations{
		Geo: &annotator.Geolocation{
			City: "Atlanta",
		},
		Network: &annotator.Network{
	...
	output2, err := parser.InsertAnnotation(fakeAnn, []byte(testStr2))
	if err != nil {
		log.Println(err)
		t.Error("Cannot insert annotation")
	}
	...
	if pttest2.Hop[0].Source.City != "Atlanta" {
...
func TestExtractIP(t *testing.T) {
	...
	testStr4 := `{"UUID": "ndt-plh7v_1566050090_000000000004D60F"}
	{"type":"cycle-start", "list_name":"/tmp/scamperctrl:51803", "id":1, "hostname":"ndt-plh7v", "start_time":1566691268}
