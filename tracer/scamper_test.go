...
func TestConvertTrace(t *testing.T) {
	...
	_, err := ConvertTrace([]byte(testStr))
	if err != nil {
	...
	*IPServiceSocketFilename = dir + "/ipservice.sock"
	srv, err := ipservice.NewServer(*IPServiceSocketFilename,
	...
	output, err := ConvertTrace([]byte(testStr))
